...
exports.run = (client, message, args) => {
  ...
  if (_drug != undefined) {
    ...
    let substanceRequest = function(data) {
      return request("https://api.psychonautwiki.org", query).then(data => { return data } );
    }
    ...
    results.then(function(data) {
      ...
      if (roa.dose) {
        ...
        //check for existence of threshold or don't print threshold
        if (roa.dose.threshold) {
          dosage_message += "Threshold: " + roa.dose.threshold + roa.dose.units + "\n";
        }

        dosage_message += "Light: " + roa.dose.light.min + "-" + roa.dose.light.max + roa.dose.units + "\n" +
        "Common: " + roa.dose.common.min + "-" + roa.dose.common.max + roa.dose.units + "\n" +
        "Strong: " + roa.dose.strong.min + "-" + roa.dose.strong.max + roa.dose.units + "\n" +
        "Heavy: " + roa.dose.heavy + roa.dose.units + "\n" + "```\n";
      }
      ...
      if (roa.duration) {
        ...
        if (roa.duration.total) {
          duration_message += "Total: " + roa.duration.total.min + "-" + roa.duration.total.max + " " + roa.duration.total.units + "\n";
        }

        if (roa.duration.onset) {
          duration_message += "Onset: " + roa.duration.onset.min + "-" + roa.duration.onset.max + " " + roa.duration.onset.units + "\n";
        }

        if (roa.duration.comeup) {
          duration_message += "Comeup: " + roa.duration.comeup.min + "-" + roa.duration.comeup.max + " " + roa.duration.comeup.units + "\n";
        }

        if (roa.duration.peak) {
          duration_message += "Peak: " + roa.duration.peak.min + "-" + roa.duration.peak.max + " " + roa.duration.peak.units + "\n";
        }

        if (roa.duration.offset) {
          duration_message += "Offset: " + roa.duration.offset.min + "-" + roa.duration.offset.max + " " + roa.duration.offset.units + "\n";
        }
      ...
      if (substance.tolerance) {
        var tolerance_message = "**Tolerance**```\n" +
        "Full: " + substance.tolerance.full + "\n" +
        "Half: " + substance.tolerance.half + "\n" +
        "Baseline: " + substance.tolerance.zero + "```";  
      } else {
