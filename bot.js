...
client.on('message', message => {
	...
	if (message.content.toLowerCase().includes("-lsdtolerance")) { // Usage -lsdtolerance [days since last trip]. calculates tolerance/extra dose needed to achieve normal effects
		...
		message.channel.send("Take ~"+Math.ceil(y / 10) * 10+"% of the drug to reach full effects.");
	...
	if (message.content.toLowerCase().includes("-info"))
	{
		var str = message.content;
		var res = str.split(" ");
		var _drug = str.toLowerCase().replace("-info ", "", -1).replace(/-/g, "", -1).replace(/ /g, "", -1); //removes all symbols and puts everything in lower case so bot finds the images easier
		...
		if(_drug == "morningglory") {_drug = "lsa";}
		if(_drug == "hawaiianbabywoodrose") {_drug = "lsa";}
		...
		if (message.content.toLowerCase().includes("dxm")){
			setTimeout(function(){
				message.channel.send("To calculate DXM dose for weight do:\n```-dxmcalc [weight in pounds]```");
			}, 1500);
		}
	}
